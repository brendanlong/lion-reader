...
-- Step 1: Update all feeds with rss/atom/json type to 'web'
UPDATE "feeds" SET "type" = 'web' WHERE "type" IN ('rss', 'atom', 'json');

--> statement-breakpoint

-- Step 2: Update all entries with rss/atom/json type to 'web'
UPDATE "entries" SET "type" = 'web' WHERE "type" IN ('rss', 'atom', 'json');

--> statement-breakpoint

-- Step 3: Drop the constraint that references old types
ALTER TABLE "entries" DROP CONSTRAINT "entries_last_seen_only_fetched";

--> statement-breakpoint

-- Step 4: Add the updated constraint using 'web' instead of rss/atom/json
ALTER TABLE "entries" ADD CONSTRAINT "entries_last_seen_only_fetched"
  CHECK ((type = 'web') = (last_seen_at IS NOT NULL));

--> statement-breakpoint

-- Step 5: Drop the partial index that references old types
