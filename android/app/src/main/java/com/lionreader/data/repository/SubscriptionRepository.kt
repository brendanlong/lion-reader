...
sealed class SyncResult {
    ...
    data class Error(val code: String, val message: String) : SyncResult()
...
class SubscriptionRepository @Inject constructor(
     ...
     */
    fun getSubscriptions(): Flow<List<SubscriptionWithFeed>> {
        return subscriptionDao.getAllWithFeeds()
    }
     ...
     */
    fun getSubscription(subscriptionId: String): Flow<SubscriptionWithFeed?> {
        return subscriptionDao.getSubscriptionWithFeed(subscriptionId)
    }
    ...
    fun getTotalUnreadCount(): Flow<Int> {
        return subscriptionDao.getTotalUnreadCount()
    }
    ...
    suspend fun syncSubscriptions(): SyncResult {
        return when (val result = api.listSubscriptions()) {
    ...
    private suspend fun updateLocalDatabase(subscriptions: List<SubscriptionDto>) {
        ...
        val feeds = subscriptions.map { dto ->
            FeedEntity(
                id = dto.feed.id,
                type = dto.feed.type,
                url = dto.feed.url,
        ...
        val subscriptionEntities = subscriptions.map { dto ->
            SubscriptionEntity(
                id = dto.id,
                feedId = dto.feedId,
