...
import { extractTextFromHtml } from "@/server/feed/entry-processor";

describe("extractTextFromHtml", () => {
  describe("basic text extraction", () => {
    it("extracts plain text from simple HTML", () => {
      const html = "<p>Hello world</p>";
      expect(extractTextFromHtml(html, 300)).toBe("Hello world");
    });

    it("returns empty string for empty input", () => {
      expect(extractTextFromHtml("", 300)).toBe("");
    });

    it("returns empty string for whitespace-only HTML", () => {
      expect(extractTextFromHtml("<p>   </p>", 300)).toBe("");
    });

    it("handles plain text without HTML tags", () => {
      const text = "Just some plain text";
      expect(extractTextFromHtml(text, 300)).toBe("Just some plain text");
    });
  });
  ...
  describe("spacing between block elements", () => {
    it("adds space between heading and paragraph", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("The Title The content starts here.");
    });

    it("adds space between consecutive paragraphs", () => {
      const html = "<p>First paragraph.</p><p>Second paragraph.</p>";
      expect(extractTextFromHtml(html, 300)).toBe("First paragraph. Second paragraph.");
    });

    it("adds space after list items", () => {
      const html = "<ul><li>Item one</li><li>Item two</li></ul>";
      expect(extractTextFromHtml(html, 300)).toBe("Item one Item two");
    });

    it("handles br tags", () => {
      const html = "Line one<br>Line two<br/>Line three";
      expect(extractTextFromHtml(html, 300)).toBe("Line one Line two Line three");
    });

    it("handles hr tags", () => {
      const html = "<p>Before</p><hr><p>After</p>";
      expect(extractTextFromHtml(html, 300)).toBe("Before After");
    });
    ...
    it("handles deeply nested block elements", () => {
      ...
      `;
      expect(extractTextFromHtml(html, 300)).toBe("Article Title First paragraph.");
    });
  });
  ...
  describe("whitespace normalization", () => {
    ...
    it("does not add duplicate spaces between block elements", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("Title Content");
    });
  });
  ...
  describe("script and style exclusion", () => {
    ...
    it("handles nested script tags correctly", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("A B");
    });
  });
  ...
  describe("HTML entity decoding", () => {
    it("decodes common HTML entities", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("Rock & Roll");
    });

    it("decodes numeric entities", () => {
      const html = "<p>&#60;tag&#62;</p>";
      expect(extractTextFromHtml(html, 300)).toBe("<tag>");
    });

    it("decodes named entities", () => {
      const html = "<p>&quot;quoted&quot; &mdash; dashed</p>";
      expect(extractTextFromHtml(html, 300)).toBe('"quoted" — dashed');
    });
  });
  ...
  describe("truncation", () => {
    it("does not truncate text within limit", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("Short text");
    });

    it("truncates at word boundary with ellipsis", () => {
      const html = "<p>This is a longer piece of text that needs truncating.</p>";
      const result = extractTextFromHtml(html, 30);
    ...
    it("handles real-world summary case", () => {
      ...
      `;
      const result = extractTextFromHtml(html, 300);
  ...
  describe("inline elements", () => {
    it("preserves text from inline elements without extra spacing", () => {
      ...
      expect(extractTextFromHtml(html, 300)).toBe("This is bold and italic text.");
    });

    it("handles links", () => {
      const html = '<p>Click <a href="http://example.com">here</a> to continue.</p>';
      expect(extractTextFromHtml(html, 300)).toBe("Click here to continue.");
    });

    it("handles spans", () => {
      const html = '<p>Some <span class="highlight">highlighted</span> text.</p>';
      expect(extractTextFromHtml(html, 300)).toBe("Some highlighted text.");
    });
  });
});
