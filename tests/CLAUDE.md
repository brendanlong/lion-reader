...
- **Component ↔ tRPC integration** (components that embed `useQuery`/`useMutation`): rendered in jsdom via `renderWithTrpc` in `tests/utils/component-test-helpers.tsx`, which wraps the component in the real `trpc.Provider` + `QueryClientProvider` but swaps the HTTP link for the terminating **handler link** (`createHandlerLink` in `src/lib/trpc/handler-link.ts` — the same link the public demo runs on) that resolves each procedure from a `{ "router.procedure": handler }` map (no MSW, no internal mocks). The returned `calls`/`callsFor(path)` let a test assert which procedures ran with which input. See `tests/unit/frontend/components/{EntryContent,EditSubscriptionDialog,Sidebar}.test.tsx`; `DemoReader.test.tsx` drives the whole reader tree through the demo store's handlers, so it doubles as an end-to-end check of the entries/sidebar wiring without a database. Because there's no HTTP layer, handler return values reach hooks un-serialized (Dates stay Dates); provide handlers for **every** procedure the subtree issues (unhandled paths error loudly).
