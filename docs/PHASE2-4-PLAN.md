...
## Phase 5: Read-it-Later & Curation

### 5.1 Saved Articles (Read-it-Later)

**Dependencies**: Content cleaning (Readability) - already implemented

**Use Case**: Personal queue of articles to read later. High-volume, transient - user processes and archives items like an inbox.
...
CREATE TABLE saved_articles (
  ...
  archived_at timestamptz,  -- soft delete / "done"
...
CREATE INDEX idx_saved_articles_unread ON saved_articles(user_id)
  WHERE NOT read AND archived_at IS NULL;
````

**API Endpoints**:

```
POST   /v1/saved              { url } → { article }  -- Save URL
GET    /v1/saved              → { items: Article[], nextCursor? }
GET    /v1/saved/:id          → { article }
PATCH  /v1/saved/:id          { read?, archived? } → { article }
DELETE /v1/saved/:id          → {}
POST   /v1/saved/mark-read    { ids: string[] } → {}  -- Bulk mark read
```

**Implementation**:

1. **Save flow**:
   - Accept URL, fetch page
   - Extract metadata (title, og:image, site name)
   - Run Readability for content
   - Store in saved_articles
...
2. **Bookmarklet**:
   ...
   javascript: (function () {
     ...
     window.open("https://lionreader.com/save?url=" + url, "_blank", "width=400,height=300");
   })();
   ```

3. **Save page** (`/save?url=...`):
   - Minimal UI - shows saving progress
   - Auto-closes on success (or shows error)
   - Optional: browser extension for better UX

**UI**:

- "Saved" section in sidebar (like Starred)
- Same entry list UI as feeds
- Archive button (removes from list but keeps in DB)
- Keyboard shortcut: `g` then `l` (go to saved/later)

---

### 5.2 Curated Lists (Future - Social)

**Dependencies**: Saved Articles

**Use Case**: Shareable collections of articles. Low-volume, persistent - user maintains themed lists for others to follow.
...
CREATE TABLE lists (
  ...
  name text NOT NULL,
  description text,
  slug text,  -- for public URL: /lists/:username/:slug
...
CREATE TABLE list_items (
  ...
  -- Can reference either a saved article OR a feed entry
  saved_article_id uuid REFERENCES saved_articles ON DELETE CASCADE,
  ...
  -- Must have exactly one of saved_article_id or entry_id
  CONSTRAINT one_item_type CHECK (
    (saved_article_id IS NOT NULL AND entry_id IS NULL) OR
    (saved_article_id IS NULL AND entry_id IS NOT NULL)
  )
);
...
```
GET    /v1/lists              → { items: List[] }
POST   /v1/lists              { name, description?, isPublic? } → { list }
PATCH  /v1/lists/:id          { name?, description?, isPublic? } → { list }
DELETE /v1/lists/:id          → {}

GET    /v1/lists/:id/items    → { items: ListItem[] }
POST   /v1/lists/:id/items    { savedArticleId? | entryId?, note? } → { item }
PATCH  /v1/lists/:id/items/:itemId  { note?, position? } → { item }
DELETE /v1/lists/:id/items/:itemId  → {}
```

**Key Differences from Saved Articles**:

| Saved Articles                | Curated Lists           |
| ----------------------------- | ----------------------- |
| Personal queue                | Shareable collections   |
| Flat list                     | Multiple named lists    |
| Transient (archive when done) | Persistent (maintained) |
| URLs only                     | URLs + feed entries     |
| Private only                  | Public or private       |

**Why Separate?**:

- Bookmarklet must be instant - no decisions
- Tags are for subscriptions, not individual articles
- "Read later" and "recommend to others" are different mental models
...
7. **saved_articles table** - New table for read-it-later
