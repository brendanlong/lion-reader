...
- **Subscribe time**: rows are inserted for entries currently in the feed — those with `entries.last_seen_at >= feeds.last_entries_updated_at` — so a new subscriber sees current feed contents but not arbitrarily old ones (disappeared entries fall below `last_entries_updated_at` and stay excluded). See `createSubscription` in `src/server/services/subscriptions.ts`. The comparison is `>=`, not `=`, to also cover entries a **WebSub hub pushed since the last poll**: a push stamps `last_seen_at = pushTime` but deliberately leaves `last_entries_updated_at` at the last poll (and no longer bumps `last_fetched_at`), so pushed entries sit above `last_entries_updated_at` and strict equality would hide them — a push-active feed's new subscriber would see an **empty feed** (issue #1078). For a non-WebSub feed nothing is ever stamped above `last_entries_updated_at`, so `>=` is exactly `=` and behavior is unchanged. To keep this ground-truthed, the interactive subscribe path (`subscriptions.create`) first **forces a fresh fetch of a stale feed** through the shared fetch path (`handleFetchFeed({ feedId }, { inline: true })`) when `shouldRefetchOnSubscribe` says so — a WebSub feed whose last real poll is older than the cache window, or a normal feed past its poll cadence. That refresh re-stamps every current entry to one generation (`alwaysUpdateVisibility`), so `>=` reflects exactly the current feed and excludes anything the publisher removed since a push; it's best-effort (a failed fetch falls back to the cached entries) and skipped for in-cadence feeds, keeping normal subscribes network-free. The `inline` refresh runs the identical fetch/process/fan-out code as the background poll (just off the event loop, unconditional, and skipping full-content fetch), so points 2–3 below apply to it too.
