...
The whole OAuth/MCP surface also answers URLs with a **trailing slash in place** (`POST /api/mcp/`, `/oauth/token/`, …): `skipTrailingSlashRedirect` is enabled in `next.config.ts` and `src/proxy.ts` rewrites slashed OAuth/MCP paths instead of redirecting, because server-to-server OAuth clients (claude.ai's connector uses python-httpx) don't follow redirects on POST and claude.ai has been observed appending trailing slashes (upstream #324). The proxy reproduces Next's default 308 redirect for every other slashed path.
