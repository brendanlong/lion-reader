...
**In-flight transition (issue #1117)**: `user_entries` now carries denormalized `subscription_id` (1:1 attribution; NULL = saved/uploaded article, `ON DELETE SET NULL`) and `is_spam` (immutable copy of `entries.is_spam`) columns, added in migration `0086`. When visibility later flips onto this column, the saved-article arm must keep gating on the entry **type** (as the view does today), never on `subscription_id IS NULL` — otherwise a NULL-attributed row would become visible fail-open. They are **informational only for now — visibility is still resolved through `subscription_feeds`**, and nothing may read them for visibility or counts until a later release flips over (after reconciling stamped vs junction-derived values in production; the reconciliation query lives in `tests/integration/user-entries-subscription-id.test.ts`). Consistency is maintained three ways: the bulk insert paths (feed fanout, subscribe-time populate) set the columns inline; every other insert path relies on the `user_entries_fill_denormalized` BEFORE INSERT trigger (which also fills the timeline sort key, and makes previous-release writers safe during rollout); and the feed-merge job re-stamps rows from the old subscription to the survivor. The planned end state: visibility becomes `user_entries JOIN subscriptions` directly, `subscription_feeds` and the `COUNT(DISTINCT)` dedup are deleted, and trigger-maintained unread counters become additive per subscription (spam permanently excluded from counts).
