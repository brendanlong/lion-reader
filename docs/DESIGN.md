...
- **Google Reader API** (`/api/greader.php/*`): compatibility layer for Google Reader clients. Every integer id a client sees is a **stored serial**, not derived from a UUID (issue #1117): item ids (`entries.greader_item_id`), feed stream ids (`subscriptions.greader_stream_id` / `feeds.greader_stream_id`), tag sortids (`tags.greader_sortid`), and user ids (`users.greader_user_id`). So an id is read straight off its row and the reversible lookups are unique-index seeks — `greaderItemIdsToUuids` for items, `feedStreamIdToSubscriptionUuid` / `resolveFeedStream` for feed streams — with no `uuidToInt64` projection or timestamp-window candidate scan. Subscription and feed stream ids draw from **one shared sequence** (`greader_id_seq`), so they're globally unique and a `feed/{int}` resolves to at most one of {subscription, saved feed}. Saved (read-it-later) articles have no subscription row, so they are exposed to Google Reader clients as a synthetic uncategorized "Saved Articles" subscription keyed by the saved feed's own stream serial — see issue #730. `resolveFeedStream` maps that stream id back to `type='saved'`, `formatEntryAsItem` gives saved items that feed as their `origin.streamId`, and `subscription/list` + `unread-count` include the synthetic feed. A subscription (not a folder) avoids the folder-name-uniqueness edge cases a synthetic folder would create.
- **Wallabag API** (`/api/wallabag/*`): compatibility layer for Wallabag read-it-later clients. Its integer ids reuse the same stored serials (issue #1117): entry id = `entries.greader_item_id` (reversed by `resolveWallabagEntry`, a unique-index seek scoped through `visible_entries`), tag id = `tags.greader_sortid`, user id = `users.greader_user_id` (both opaque, never reversed). This replaced a 31-bit SHA-256 hash of the entry UUID that had real user-visible collisions (the legacy `entries.wallabag_id` generated column was dropped in migration 0098).
...
| `resanitize.ts`         | `selectStaleEntriesForResanitize`, `sanitizeFamilyFromRaw`, `persistResanitizedFamily` - shared primitives for re-sanitizing stored entry HTML after a `SANITIZER_VERSION` bump (read-path self-heal + manual `scripts/resanitize-bulk.ts`) |
