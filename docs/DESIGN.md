...
Setting `LOG_MCP_REQUESTS=true` makes `src/proxy.ts` emit one structured line per request to the OAuth/MCP surface (host, method, path, **redacted** query, user-agent, and `hasAuthorization` — a boolean, never the token). This is the diagnostic that separates claude.ai's failure modes — e.g. an authenticated `initialize` arriving with no Bearer header (the connector header-drop bug) vs. a registration/discovery failure.
