...
- **user_feeds** - Subscriptions with feed metadata merged, using subscription ID as the primary key. Display-only (subscription list surfaces); link/ownership/scoping checks must query `subscriptions` directly
...
**In-flight transition (issue #1117)**: `user_entries` carries denormalized `subscription_id` (1:1 attribution; NULL = saved/uploaded article, `ON DELETE SET NULL`) and `is_spam` (immutable copy of `entries.is_spam`) columns, added in migration `0086` and backfilled/verified against the junction-derived attribution in production. As of migration `0087`, **`visible_entries` resolves visibility through `ue.subscription_id` directly** — the view emits exactly one row per (user, entry), so consumers use plain `count(*)` (no more `COUNT(DISTINCT)`/`DISTINCT ON` dedup). The saved-article arm gates on the entry **type**, never on `subscription_id IS NULL`, so a NULL-attributed row stays hidden unless starred (fail-closed). Consistency is maintained three ways: the bulk insert paths (feed fanout, subscribe-time populate) set the columns inline; every other insert path relies on the `user_entries_fill_denormalized` BEFORE INSERT trigger (which also fills the timeline sort key); and the feed-merge job re-stamps rows from the old subscription to the survivor. The `subscription_feeds` junction table was **dropped in migration `0091`** — every path (visibility, filters, counts, fanout dedup, sync, Google Reader, subscribe, email ingest, the merge job) works purely off `user_entries.subscription_id`, and feed merges rely solely on the merge-job re-stamp (which also preserves attribution across multi-hop merge chains). As of migration `0092` (step 5a), four **denormalized unread counters** exist — `subscriptions.unread_count`/`starred_unread_count` and `users.saved_unread_count`/`starred_unread_count` — maintained by the `user_entries_counters_*` statement-level triggers (transition tables, so bulk writes cost one grouped update per statement) with **spam permanently excluded**. Step 5b (migration `0093`, which also exposes `unread_count` through `user_feeds`) flipped every badge/count read to counter arithmetic (`src/server/services/counts.ts`, `tags.ts`, `subscriptions.ts`, the Google Reader saved feed): subscription = `unread_count`, tag/uncategorized = SUM over the tag's/untagged ACTIVE subs, saved/starred = the users-row counters, and `all` = SUM(unread) over active subs + saved + SUM(starred_unread) over inactive subs — the last term is the starred-orphans correction, derived at read time so unsubscribe/resubscribe/merge need zero counter writes. Badges therefore **exclude spam everywhere**. Only the generic filtered counts (`countEntries`/`countTotalEntries`, used by MCP/Wallabag) remain scans; the transition is essentially complete. The daily `reconcile_counters` singleton job recomputes from ground truth, fixes drift, and logs any fix at error level (a fix means a trigger bug — investigate).
