...
- **Wallabag API** (`/api/wallabag/*`): compatibility layer for Wallabag read-it-later clients. Its integer ids reuse the same stored serials (issue #1117): entry id = `entries.greader_item_id` (reversed by `resolveWallabagEntry`, a unique-index seek scoped through `visible_entries`), tag id = `tags.greader_sortid`, user id = `users.greader_user_id` (both opaque, never reversed). This replaced a 31-bit SHA-256 hash of the entry UUID that had real user-visible collisions; the legacy `entries.wallabag_id` generated column survives unreferenced until a follow-up migration drops it.
