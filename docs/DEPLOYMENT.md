...
- **CDN**: the deploy workflow has no CDN steps. A single Bunny pull zone (`https://lionreader.b-cdn.net`, also `https://cdn.lionreader.com`) has the app as origin and wraps the **whole site**, but honors origin `Cache-Control`, so what it caches is decided by our headers — configured by the `ASSET_PREFIX` build arg (and its `NEXT_PUBLIC_ASSET_PREFIX` twin) defaulted in the `Dockerfile`. It serves two things: (1) the hashed `/_next/static` assets via Next's `assetPrefix`, content-hashed + `immutable`, so no purging or upload ordering is needed; and (2) the demo hero/OG images (`public/demo/*`), which are **not** filename-hashed, so `demoImageUrl` appends a `?v=<content-hash>` cache-buster (generated manifest, `pnpm generate:demo-images`) — **Bunny must include `v` in its cache-key query params** (it already keys on `entry` and `_rsc`) for that busting to work. HTML/RSC is never CDN-cached: dynamic pages keep Next's default `private, no-store`, and the statically-prerendered public pages' `s-maxage` is rewritten to `private, no-cache` in the custom server (see below). The pull zone must send CORS headers (Bunny's "CORS headers" option) so cross-origin font loads work.
...
- **RSC payloads.** Soft (`?_rsc=`) navigations fetch a Flight payload tied to the current build id; a cached shell from an older build hitting a newer origin version-skews. Hard navigations avoid _this_ particular problem — but not the JS-bundle one above. Note that `?_rsc=<hash>` is a **router-state** cache-buster, not a build/deploy id, so it does **not** make an RSC payload safe to shared-cache across deploys — the same route+state hashes identically on both builds. Pinning to a deploy would require Next's `deploymentId`/`?dpl=`, which we don't set.

**Enforcement.** Next stamps `Cache-Control: s-maxage=31536000` on the statically-prerendered `(public)` pages **and their RSC payloads** — a year-long _shared_-cache lifetime for exactly the build-coupled content described above. Because our pull zone wraps the whole site and honors that header (and already keys on `_rsc`/`entry`), the custom server rewrites it to `private, no-cache` before it leaves the origin (`neutralizeStaticSharedCache`, `src/server/http/static-cache-control.ts`; unit-tested in `tests/unit/static-cache-control.test.ts`). This also closes the maintenance-gate bypass (#1318): an edge-cached `/login`/`/register` would serve a 200 while the gate is trying to 503 everything DB-touching. `no-cache` (not `no-store`) still lets the browser hold a copy, but it must revalidate against the origin before use — so a deploy can't leave a browser booting a stale document (Next does not self-heal missing bootstrap chunks on an initial load), and a revalidation during maintenance hits the 503 gate.
