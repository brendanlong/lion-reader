...
- **CDN purge**: after a successful rollout, the workflow purges the Bunny CDN pull zone. Every deploy deletes the previous build's hashed `/_next/static` assets, so any HTML the CDN cached before the deploy references chunks that now 404 — hydration then fails and visitors get the global-error page (issue #1350). The purge makes the CDN refetch fresh HTML immediately; the storage-zone asset upload (below) keeps old builds' assets servable for any HTML that lingers. Requires the optional `BUNNY_API_KEY` **secret** (Bunny dashboard > Account Settings > API) and `BUNNY_PULL_ZONE_ID` **repository variable** (the numeric pull zone id from the zone's dashboard URL; not sensitive, so it lives under Settings > Secrets and variables > Actions > Variables); the step skips gracefully without them.
- **Static assets to Bunny storage** (issue #1318): with the optional `BUNNY_STORAGE_ZONE_NAME` **repository variable** + `BUNNY_STORAGE_ZONE_PASSWORD` **secret** set (plus the `BUNNY_STORAGE_ENDPOINT` variable if the zone's primary region differs from the workflow's Los Angeles default — e.g. `https://ny.storage.bunnycdn.com`), the deploy splits into build → extract `/_next/static` from the built image → upload to the storage zone (`scripts/upload-bunny-static.sh`) → release, so every asset is uploaded **before** any HTML referencing it goes live. Uploads are additive — old builds' hashed assets stay servable forever, which is the durable fix for the deploy-skew 404s of issue #1350. Serving them requires a pull zone backed by the storage zone and an Edge Rule on the main pull zone routing `/_next/static/*` to it (managed in Terraform; see issue #1318). Without the storage config the workflow uses the original single-step `flyctl deploy`.
