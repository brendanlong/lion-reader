...
Sanitizing a large body is the dominant cost of `entries.get` (~50ms per ~700KB, ×4 fields), so the read path uses `sanitizeEntryHtmlCached` from `@/server/html/sanitize-cache` — a content-addressed Redis cache (`sha256(html)` + `SANITIZER_VERSION`) that degrades to a direct sanitize when Redis is absent. **Bump `SANITIZER_VERSION` in `sanitize.ts` whenever `SANITIZE_OPTIONS` change**, or stale sanitization will be served. The feed worker and full-content fetch proactively `warmSanitizedEntryHtml(...)` at write time so the user's first read is a cache hit.
