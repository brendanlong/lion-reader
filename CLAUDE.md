...
- **Cache logic** (`src/lib/cache/`): pure functions, unit-tested in `tests/unit/frontend/cache/` using the factories in `tests/utils/cache-test-helpers.ts` — a real `QueryClient` for query data plus a hand-rolled tRPC-utils mock (`tests/utils/trpc-mock.ts`; see #957). Add cases there when changing cache operations or event handling.
