...
- `pnpm test:native` / `pnpm lint:native` - `cargo test` / `cargo clippy -D warnings` across all four native crates (each is its own cargo workspace, so `scripts/cargo-native.sh` loops over them). Both gate CI.
