...
- We use soft navs everywhere (except logout) rather than hard navs because HTML is not CDN-cached, so there's no version-skew to avoid. If you're tempted to switch back to hard navs to make HTML CDN-cacheable, read "Why HTML is not CDN-cached" in `docs/DEPLOYMENT.md` first — hard navs alone don't make HTML caching safe (the old JS bundles still have to be retained), so it's not a shortcut.
