...
Shared icons live in `@/components/ui/icons` (navigation, actions, status, media, files, brand, loading, …). Check its exports before adding an inline SVG.
...
**Use the semantic color tokens instead of raw zinc light/dark pairs — one token per role, themed once.** A component writes a role (`bg-danger-subtle`), never a raw pair (`bg-red-50 dark:bg-red-950`); each token is one utility class covering all themes, and the palettes live in `src/app/globals.css` (`:root` for light, `.dark`, and `.epaper` for e-ink screens), so retheming means editing CSS variables, not call sites:
...
| `bg-surface-muted`   | `bg-zinc-100 dark:bg-zinc-800`         | Hover/selected fill (`hover:bg-surface-muted`) on controls/rows; chips          |
...
| `--focus` (CSS var)  | accent (amber-700 / amber-500)         | Drives the global `:focus-visible` outline — never style focus per-component    |
| `control-selected`   | accent (amber-700 / amber-500)         | Selected/"on" outline: checkbox/radio/selected-card `border`/`ring`, radio dots |

There is no `--text-strong`/`--text-emphasis`: near-black titles and unread indicators read as too harsh, so headings and primary values share the softer `--text-body` tone (issue #1227). Don't reintroduce a darker title token — e-paper gets its intentional near-black harshness from its own darker `--text-body` value.

#### Status colors (danger / success / warning / info / star)

**Never hand-roll a raw `red/green/amber/yellow` pair for a status.** Each status role is tokenized the same way as the neutrals — one utility class per role, covering light, dark, and e-paper — themed in `globals.css` and exposed via `@theme inline`. Dark mode keeps status hues conventional (a red error stays red); e-paper darkens the text/icon tones so they survive grayscale and drops the `-subtle` fills to paper white (see the e-paper bullet under Guidelines). The base on-surface text and `-subtle-foreground` tokens meet WCAG AA on their backgrounds in all three themes (this is why success text is `green-700`, not the AA-failing `green-600`). The per-role `-hover`/`-solid-hover` tokens are more than each role strictly needs, but they're cheap and symmetric — keep them rather than churning call sites to trim them.
...
| `bg-{role}-subtle`              | `bg-danger-subtle`              | `bg-success-subtle`               | `bg-warning-subtle`               | Subtle status card/alert/badge fill (also `hover:` fills); white on e-paper |
...
- **`info`** is the blue trio (`text-info-foreground`, `bg-info-subtle`, `border-info-border`, `text-info-subtle-foreground`, …). Use it for genuinely informational (not brand) blue: the `WebSub`/websub-active badges, the invite `pending` badge, the "info" `SummaryCard` variant, and the Google-Docs lock icon are all `info`, not raw blue/purple. The AI summary card stays in this blue family on purpose: against the amber chrome a blue "assistant" box reads as clearly machine-generated, and a violet "AI" hue would be a needless fourth family. Links use `text-accent hover:text-accent-hover` like every other link. The accent is the warm **amber** brand hue in every theme (amber-700 light / amber-500 dark / amber-800 e-paper) — warm ⇒ low-blue-light, and deliberately kept distinct from `info` blue (informational) and `danger` red (error): amber = brand/interactive, blue = informational, red = error.
...
- **`warning` and the accent are both amber — the one deliberate overlap.** Accepted because they never share a **treatment**: the accent appears as link text, dots, and focus; `warning` always as an icon plus a bordered subtle box (`Alert`/`StatusCard warning`, always with `AlertIcon`). If the two ever read as ambiguous, the single lever is nudging `--warning` toward a golder yellow in `globals.css` — one file, no call sites — not fragmenting `warning` into a second hue at the call sites.
...
- **Active nav rows stay neutral** — `NavLink`'s active state is `bg-surface-muted`, not the accent. Deliberate: the accent is for links/dots/focus, not for "where you are".
...
- **Use existing icons** - Check `@/components/ui/icons` before adding inline SVGs
...
- **Dark mode** - All UI components support dark mode via `dark:` Tailwind classes. The e-paper theme (`.epaper` on `<html>`) is light-like, so `dark:` variants don't apply to it; it restyles the app purely through the semantic-color CSS variables (see the `.epaper` block in `globals.css`). When a component genuinely needs an e-paper-only override, use the `epaper:` Tailwind variant — registered via `@custom-variant epaper` in `globals.css`, mirroring `dark:`. Reach for it sparingly; prefer the semantic tokens.
- **E-paper spends contrast on borders, never on fills.** An e-ink panel dithers any tone between its few flat gray levels, so a "subtle" fill arrives as a sandy cross-hatch: on e-paper every surface (including `surface-muted`, i.e. every hover/selected fill) is paper white, every `-subtle` status fill is white, every `edge`/`-border` token is black, and "on"/primary is a black fill. State is therefore carried by the **border**, which has two consequences for new components:
  - A control that's borderless in light/dark needs **`control-outline`** (globals.css) so it gets that black box on e-paper. It reserves a transparent 1px border in every theme, so the box is the same size in all three and switching theme reflows nothing. Its companion `control-outline-none` is the unpainted half of a control outlined only when current (sidebar rows). Rationale and the layering rules are in the `globals.css` comment.
  - Anything that distinguished itself by a **fill** (a selected radio card's `bg-zinc-50`, a gray chip) needs a border story on e-paper instead, and anything already distinguished by border **color** needs a second step, since `border-edge`/`-edge-strong`/`-edge-input` and every `border-{status}-border` are all the same black there. The convention: black edge = active/selected/unread, `border-fill-muted` (the theme's non-text gray) = passive/unselected/read.
