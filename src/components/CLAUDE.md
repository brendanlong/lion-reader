...
- **E-paper spends contrast on borders, never on fills.** An e-ink panel dithers any tone between its few flat gray levels, so a "subtle" fill arrives as a sandy cross-hatch: on e-paper every surface (including `surface-muted`, i.e. every hover/selected fill) is paper white, every `-subtle` status fill is white, every `edge`/`-border` token is black, and "on"/primary is a black fill. State is therefore carried by the **border**, which has two consequences for new components:
  - A control that's borderless in light/dark needs **`control-outline`** (globals.css) so it gets that black box on e-paper. It reserves a transparent 1px border in every theme, so adding it never reflows anything. Its companion `control-outline-none` is the unpainted half of a control outlined only when current (sidebar rows).
  - Anything that distinguished itself by a **fill** (a selected radio card's `bg-zinc-50`, a gray chip) needs a border story on e-paper instead, and anything already distinguished by border **color** needs a second step, since `border-edge`/`-edge-strong`/`-edge-input`/`border-{status}` are all the same black there. The convention: black edge = active/selected/unread, `border-fill-muted` (the theme's non-text gray) = passive/unselected/read.
