      ...
      <nav className="flex h-full flex-col bg-white dark:bg-zinc-900">
        ...
        <div className="space-y-1 p-3">
          <Link
            href="/all"
            onClick={handleClose}
            className={`flex min-h-[44px] items-center justify-between rounded-md px-3 py-2 text-base font-medium transition-colors ${
              isActiveLink("/all")
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {subscriptionsQuery.isLoading ? (
            ...
            <p className="px-3 text-base text-red-600 dark:text-red-400">Failed to load feeds</p>
          ) : subscriptionsQuery.data?.items.length === 0 ? (
            <p className="px-3 text-base text-zinc-500 dark:text-zinc-400">
                  ...
                  <li key={tag.id}>
                    ...
                    <div className="flex min-h-[44px] items-center">
                      ...
                      <Link
                        href={tagHref}
                        onClick={handleClose}
                        className={`flex min-h-[44px] flex-1 items-center gap-2 rounded-md px-3 py-2 text-base transition-colors ${
                          isActive
                            ? "bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-50"
                            : "text-zinc-700 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800"
                        }`}
                      >
                        ...
                        {tag.unreadCount > 0 && (
                          <span className="ml-auto shrink-0 text-sm text-zinc-500 dark:text-zinc-400">
                            ({tag.unreadCount})
                          </span>
                        )}
                      </Link>
                          ...
                          return (
                            <li key={sub.id} className="group relative">
                              <Link
                                href={subHref}
                                onClick={handleClose}
                                className={`flex min-h-[44px] items-center justify-between rounded-md px-3 py-2 text-base transition-colors ${
                                  isSubActive
                                    ? "bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-50"
                                    : "text-zinc-700 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800"
                                }`}
                              >
                                ...
                                {sub.unreadCount > 0 && (
                                  <span className="shrink-0 text-sm text-zinc-500 group-hover:hidden dark:text-zinc-400">
                                    ({sub.unreadCount})
                                  </span>
                                )}
                              </Link>
