...
| `entry_state_changed`  | Direct: `entries.get`, `entries.list` (read, starred). Sets absolute counts via `setBulkCounts` when server provides `counts`. Fallback (sync polling): delta from list cache. |
