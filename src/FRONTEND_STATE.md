...
Full-text search is backed by the stored, GIN-indexed `entries.search_vector` column (migration 0105, #1249) — `searchEntries` reads it for both the `@@` match and the `ts_rank` relevance sort, so a query no longer tokenizes the user's history on the fly.
