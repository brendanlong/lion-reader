...
| `entry_state_changed`  | Direct: `entries.get`, `entries.list` (read, starred); `restoreUnreadEntriesToListCaches` for entries becoming unread; absolute counts via `setEntryRelatedCounts`. Always carries `counts`. The Google Reader / Wallabag compat routes pass the service `returnCounts: false`, which skips the `visible_entries` aggregation only when nothing changed (no event published) — a published event is unchanged, so this is invisible to the UI.                                                                                                                                    |
