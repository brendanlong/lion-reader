...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `new_entry`            | Direct: `subscriptions.list` unreadCount, `tags.list` unreadCount (from server-resolved `tagIds` in the event, so collapsed/uncached tags update too — #892; falls back to cached subscription tags if absent), `entries.count`. Does NOT invalidate `entries.list`. |
