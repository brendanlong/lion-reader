...
**Sign-in and linking are different flows through the same callback route.** Each provider has exactly one registered redirect URI (`getRedirectUri`), so what the callback should do travels in the per-flow Redis state blob as a `mode`. `login` picks the account from the provider's **verified email** (`processOAuthCallback`). `link` — the Settings → Linked Accounts button, via the `auth.linkAuthUrl` mutation — attaches the provider account to whoever the **session cookie** identifies (`linkOAuthAccount`, see `src/server/services/oauth-accounts.ts`), so the provider's email need not match the account's. Routing a link through the login branch is what created a second account for a user whose Google address differed from their sign-in address (#1603), so a link callback with no valid session is an **error**, never a fallthrough to sign-in. Google carries two further modes for Docs incremental authorization (`save`, `extension-save`), which only refresh an existing link's tokens and scopes.
...
## OAuth `state` Binding Cookie (login-CSRF defense)

The OAuth `state` (and Google PKCE verifier) is stored in Redis keyed by the state value, so a callback that presents a valid state only proves "we issued this state" — **not** "this is the browser that started the flow". Without more, an attacker could run their own OAuth flow, capture a valid `code`+`state` for their account, and deliver the callback URL to a victim, logging the victim into the **attacker's** account (login CSRF / session fixation, #1263). `SameSite=Lax` + one-time state + the 10-min TTL narrow but don't close this — the callback is a top-level GET, which Lax permits.

The fix is the double-submit cookie pattern (`src/server/auth/oauth/state-cookie.ts`): when an authorization URL is generated (the `{provider}AuthUrl` / `linkAuthUrl` tRPC procedures + `requestGoogleDocsAccess`) we set a short-lived `HttpOnly` cookie holding the same `state` — via the browser tRPC path's `resHeaders`, so it no-ops for REST/OpenAPI callers exactly like `setSessionCookie`. The browser-facing callback routes (`src/app/api/v1/auth/oauth/{google,apple,discord}/callback`) then require `oauthStateCookieMatches(cookie, state)` — **fail-closed** on a missing cookie — and clear the one-time cookie on every response. The **binding** is the protection (an attacker can't set/read the victim's `HttpOnly` cookie), not `SameSite`.

**One exemption: Google `extension-save` mode.** That mode's auth URL is generated in a Server Component (`src/app/(spa)/extension/save/page.tsx`), and Next.js forbids setting cookies during a Server Component render (only Server Actions / Route Handlers may). So the Google callback enforces the cookie for every mode **except** `extension-save`, which re-authorizes an already-logged-in user's own Google scopes (identified by their session) rather than logging anyone in — it isn't the login-CSRF vector. Because the Google `mode` lives in the Redis state blob, that route checks the cookie **after** consuming the state (Apple/Discord check before — no mode of theirs is exempt). If the extension-save flow is ever moved to a Route Handler, drop the exemption.

**Apple is `SameSite=None; Secure`, not Lax.** Apple uses `response_mode=form_post`, i.e. a cross-site POST from `appleid.apple.com`; `SameSite=Lax` cookies are withheld from cross-site POST navigations, so a Lax cookie would never arrive and would break every Apple login. Google/Discord redirect back with a top-level GET and stay `SameSite=Lax`. The tRPC callback mutations (`googleCallback`/`appleCallback`/`linkGoogle`/`linkApple`) are **not** cookie-guarded: they are JSON-only tRPC (CSRF-resistant), no browser flow reaches them (every provider redirects to the route handlers above), and they exist for the non-browser REST/OpenAPI surface, which takes the token from the response body.
...
All three password paths verify through `verifyPassword` (`src/server/auth/password.ts`) rather than calling `argon2.verify` directly. When the user is missing or passwordless (OAuth-only) it runs a dummy verify against a constant decoy hash so the argon2 cost is always paid — a missing account isn't measurably faster than a real password check, closing the user-enumeration timing oracle (#1267). Don't reintroduce an early return that skips argon2 before returning "invalid credentials".
