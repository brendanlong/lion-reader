...
export const subscriptionsRouter = createTRPCRouter({
   ...
   *
   * Delegates to the subscriptions service which handles:
   * 1. Checking if the feed already exists (fast path)
   * 2. Fetching the URL, discovering/parsing the feed (slow path)
   * 3. Creating the subscription (or reactivating if previously unsubscribed)
   * 4. Publishing SSE events for real-time UI updates
   *
  ...
  create: expensiveProtectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return subscriptionsService.subscribeByUrl(ctx.db, {
