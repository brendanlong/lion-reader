...
import heroImage from "./images/real-time.png";
import ogImage from "./images/real-time-og.png";
...
const article: DemoArticle = {
  ...
  contentHtml: `
    ...
    <p>Lion Reader uses <a href="https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events" target="_blank" rel="noopener noreferrer">Server-Sent Events</a> (SSE) powered by Redis pub/sub to deliver real-time updates without polling. When a feed worker fetches new content, it publishes events to Redis channels. Your browser maintains an open SSE connection that listens to these channels, receiving updates the moment they happen. This architecture is both efficient and scalable &mdash; your connection only subscribes to events for feeds you actually care about.</p>
    ...
    <ul>
      ...
      <li><strong>Per-user channels</strong> &mdash; Account-level events like subscription changes and <a href="/demo/all?entry=opml">OPML import</a> progress use a dedicated user channel (<code>user:{userId}:events</code>). This ensures actions taken in other browser sessions appear instantly across all your devices.</li>
    </ul>
