 ...
 *
 * 1. **SSR and client render must agree.** The demo prerenders the welcome
 *    article on the server and hydrates it on the client, reading the same
 *    `publishedAt` from static data. A `new Date()` evaluated at module-load
 *    time differs between the long-running server (≈ deploy time) and the
 *    browser (page load), so the article's absolute date would disagree
 *    between the two. Reading a build/deploy-stamped value instead makes both
 *    sides render the identical time.
 *
 * 2. **Welcome must stay pinned to the top** of the newest-first list, so its
 *    date must be newer than every other demo article. The deploy time always
 *    is; the dev fallback is chosen to be.
 *
