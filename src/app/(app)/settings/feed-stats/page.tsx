...
/**
 * Get feed type display text
 */
function getFeedTypeDisplay(type: "web" | "email" | "saved"): string {
  switch (type) {
    case "web":
      return "Web Feed";
    case "email":
...
function FeedStatsRow({ feed }: FeedStatsRowProps) {
  ...
  const typeDisplay = getFeedTypeDisplay(feed.type);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-4">
            ...
            />
            <StatItem icon={<TagIcon />} label="Type" value={typeDisplay} />
          </div>
        </div>
      </div>
    </div>
