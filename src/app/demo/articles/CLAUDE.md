...
set, use the **single cropped smiling-lion face** and the **`pro`** model tier:

- `input_image_path_1: assets/lion-face.png` — a clean, transparent-background
  crop of the lion's smiling face (gold face, cream muzzle, navy features,
  orange mane with cream tufts). **Use exactly one reference.** Earlier batches
  tried the logo alone (a lion behind a book, so the face isn't clearly visible)
  and then the logo + `lionreader_emojis.png` sheet together; both drifted — the
  logo doesn't show the face, and feeding two references confused the model. A
  single clean face crop is the most reliable reference.
- `model_tier: "pro"` — noticeably better reference adherence than the default
  `nb2` for this style-matching job.

In the prompt, say "match the lion's face and colors precisely to the reference
image". Worked example that produced the Text-to-Speech hero
(`public/demo/text-to-speech.png`) used an older single-logo/`nb2` recipe:

```
mode: "generate"                       # REQUIRED — see gotcha
input_image_path_1: assets/lion-face.png   # single clean face crop
...
- **Always pass `assets/lion-face.png`** as the single `input_image_path_1` so the
  lion stays on-model across articles. Don't stack multiple references.
