...
Pass the lion references as character references and force `mode: "generate"` so
the aspect ratio is honored (see gotcha below). For consistent, on-model lions
across the set, use **both** references and the **`pro`** model tier:

- `input_image_path_1: assets/logo-original.png` — overall brand style
- `input_image_path_2: assets/lionreader_emojis.png` — the emoji sheet shows the
  lion's **face** clearly (gold face, cream muzzle, navy features, orange mane
  with cream tufts), which the logo alone (a lion behind a book) doesn't. The
  first batch used only the logo on `nb2` and the faces drifted between images;
  re-running the whole set on `pro` with the emoji sheet added made them match.
- `model_tier: "pro"` — noticeably better reference adherence than the default
  `nb2` for this style-matching job.

In the prompt, say "match the lion's face and colors precisely to the two
reference images". Worked example that produced the Text-to-Speech hero
(`public/demo/text-to-speech.png`) used the single-logo/`nb2` recipe:

```
mode: "generate"                       # REQUIRED — see gotcha
input_image_path_1: assets/logo-original.png
input_image_path_2: assets/lionreader_emojis.png   # face reference; use pro tier
...
- **Always pass the logo** as `input_image_path_1` (and the emoji sheet as
  `input_image_path_2`) so the lion stays on-model across articles.
