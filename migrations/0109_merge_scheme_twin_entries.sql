...
-- is kept, and the newest last_seen_at, so it stays in the feed's current
-- generation. Per-user state folds into the survivor's row: unread if any twin
-- was unread, starred if any was starred, latest change timestamps. A user who
-- held only a loser row gets it re-pointed. The user_entries counter triggers
-- account for every step, so no counter needs recomputing.
--
-- Expand/contract: the previous release matches guids exactly, so during
-- rollout it can try to insert a twin and hit this index. That surfaces as a
-- per-entry "Failed to process entry" log line and the next fetch heals it —
-- the same outcome it already had for an identical-guid race.
--
...
-- absent. The build takes a SHARE lock on entries for its duration; on a large
-- production table pre-build it CONCURRENTLY by hand first (see CLAUDE.md), and
-- the IF NOT EXISTS below no-ops.
...
WHERE s.user_id = f.user_id
  ...
  AND ((s.read AND NOT f.all_read) OR (NOT s.starred AND f.any_starred));
--> statement-breakpoint
