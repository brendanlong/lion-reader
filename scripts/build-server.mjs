#!/usr/bin/env node
/**
 * Build script for bundling the custom Next.js server.
 *
 * Creates a single optimized JavaScript file that can run with just:
 *   node dist/server.js
 *
 * Next.js itself is kept external (loaded from node_modules at runtime)
 * since it's too large to bundle and needs its internal file structure.
 */
...
const buildOptions = {
  ...
  // - next: Too large, needs internal file structure at runtime
  // - argon2: Native module (node-gyp bindings)
  // - html-rewriter-wasm: WASM files that break when bundled
